...
.. code-block:: python
   ...
   #Recommended Setup. This is the latest stable version
   #Don't clone cowboy-dev. It's an extremely aggressive
   #dev branch for contributors, not a bleeding edge build
   git clone https://github.com/jsuarez5341/neural-mmo
   cd neural-mmo
   bash setup.sh

This will download both the OpenAI environment, the independent client, and THREE.js. The latter two are required for rendering.

The setup file will only install _external_ dependencies -- we assume you have already setup **Anaconda with Python 3.7+**. The environment is framework independently, but our experiment code (the demo) depends on PyTorch.
...
Then navigate to http://localhost:8080/forge/embyr/ in Firefox or Chrome to pull up Embyr (the renderer). Give it a few seconds to load assets and stabilize fps. Read the Embyr documentation if you are having issues. You can also view the `client source <https://github.com/jsuarez5341/neural-mmo-client>`_ if you are interested.
